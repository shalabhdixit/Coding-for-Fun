...
class ApplePayPaymentMethod(AbstractPaymentMethod):
    """Apple Pay payment method for the PaymentProcessor strategy pattern."""
    ...
    def _execute_with_retry(self, order_id: str, payment_data: dict) -> dict:
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                logger.warning(
                    "Apple Pay API timeout on attempt %d/%d for order_id=%s, retrying in %.1fs",
                ...
                delay = self._base_delay * (2 ** attempt)
                logger.warning(
                    "Apple Pay API error on attempt %d/%d for order_id=%s (%s), retrying in %.1fs",
    ...
    def _validate_api_url(url: str) -> None:
        if not url.startswith("https://"):
            raise ValueError(
                f"APPLE_PAY_API_URL must use HTTPS to protect credentials in transit. Got: '{url}'"
            )
